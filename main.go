...
func main() {
	...
	for update := range updatesChan {
		if update.CallbackQuery != nil {
			handleCallbackQuery(update, update.CallbackQuery, bot, conn)
			continue
		}
...
func handleCallbackQuery(update tgbotapi.Update, callbackQuery *tgbotapi.CallbackQuery, bot *tgbotapi.BotAPI, conn *sql.DB) {
	if callbackQuery.Message.Chat.ID == update.Message.Chat.ID {
		...
		msg := tgbotapi.NewCallbackWithAlert(callbackQuery.ID, "You are not allowed to use this keyboard")
		bot.Send(msg)
	}
}
